...
import { captureConsole, withDocsSandbox, createPackage } from './setup'
...
describe('runSync orchestrator', () => {
	test('dry-run propagates typedoc + llms overrides', async () => {
		await withDocsSandbox('runner-pass', async (sandbox) => {
			await createPackage(sandbox, `runner-pass-${process.pid}`)
			const { log } = await captureConsole(() => runSync({
	...
	test('include filter with no matches logs no packages found', async () => {
		await withDocsSandbox('runner-none', async () => {
			const { log } = await captureConsole(() => runSync({ include: ['nonexistent'], exclude: [], clean: false, hard: false, generateLlms: true, dryRun: true }), 'real')
	...
	test('exclude filter prevents processing of matching package', async () => {
		await withDocsSandbox('runner-exclude', async (sandbox) => {
			await createPackage(sandbox, `runner-exclude-${process.pid}`)
			const { log } = await captureConsole(() => runSync({ include: [], exclude: ['runner-exclude-' + process.pid], clean: false, hard: false, generateLlms: true, dryRun: true }), 'real')
	...
	test('unchanged package skipped after initial non-dry run', async () => {
		await withDocsSandbox('runner-unchanged', async (sandbox) => {
			...
			await createPackage(sandbox, name)
			await runSync({ include: [name], exclude: [], clean: false, hard: false, generateLlms: false, dryRun: false })
			const { log } = await captureConsole(() => runSync({ include: [name], exclude: [], clean: false, hard: false, generateLlms: false, dryRun: true }), 'real')
	...
	test('clean removes directories when changed (dry-run logs)', async () => {
		await withDocsSandbox('runner-clean', async (sandbox) => {
			...
			await createPackage(sandbox, name)
			await runSync({ include: [name], exclude: [], clean: false, hard: false, generateLlms: false, dryRun: false })
			await sandbox.ensureFile(path.join(sandbox.orkRoot, name, 'guides', 'index.md'), '# Changed\n')
			const { log } = await captureConsole(() => runSync({ include: [name], exclude: [], clean: true, hard: false, generateLlms: false, dryRun: true }), 'real')
	...
	test('non-dry run generates markdown API docs (index.md)', async () => {
		await withDocsSandbox('runner-md', async (sandbox) => {
			await sandbox.ensureFile(path.join(sandbox.docsDir, 'typedoc.base.json'), JSON.stringify({
			...
			await createPackage(sandbox, name)
			const { log } = await captureConsole(() => runSync({ include: [name], exclude: [], clean: false, hard: false, generateLlms: false, dryRun: false }), 'real')
			const apiDir = path.join(sandbox.docsDir, 'packages', name, 'api')
	...
	test('dry-run logs planned dependency install when autoInstallDeps=true and package has deps', async () => {
		await withDocsSandbox('runner-auto-install-dry', async (sandbox) => {
			...
			const { log } = await captureConsole(() => runSync({ include: [name], exclude: [], clean: false, hard: false, generateLlms: false, dryRun: true, typedoc: { autoInstallDeps: true } }), 'real')
